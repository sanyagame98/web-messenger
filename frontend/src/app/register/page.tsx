    ...
    <main className="flex min-h-screen items-center justify-center bg-slate-950 px-4 text-slate-100">
      <form onSubmit={handleSubmit} className="w-full max-w-sm rounded-3xl border border-slate-800 bg-slate-900 p-8 shadow-2xl">
        <div className="mb-7 text-center">
          ...
          <p className="text-sm text-slate-400">Создание аккаунта</p>
        </div>
        ...
        <input type="password" className="mb-4 w-full rounded-xl border border-slate-700 bg-slate-950 px-3 py-3 outline-none focus:border-brand-500" value={password} onChange={(e) => setPassword(e.target.value)} autoComplete="new-password" required />

        <p className="mb-4 text-xs leading-relaxed text-slate-500">Никаких SMS и кодов подтверждения. Username Roof создаст автоматически — его можно поменять в профиле.</p>
        ...
        <p className="text-center text-sm text-slate-400">Уже есть аккаунт? <Link href="/login" className="text-brand-400 hover:underline">Войти</Link></p>
      </form>
    </main>
  );
}
