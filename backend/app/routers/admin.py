...
from app.schemas import AdminPremiumGrant, AdminStarsGrant, AdminUserOut, AdminVerifiedSet
...
def grant_stars(
    ...
    _audit(db, admin, target, "grant_stars", str(payload.amount))
    db.commit()
    db.refresh(target)
...
def grant_premium(
    ...
    _audit(db, admin, target, "grant_premium", f"{payload.days} days")
    db.commit()
    db.refresh(target)
    return _admin_user(target)
...
def revoke_premium(
    ...
    _audit(db, admin, target, "revoke_premium", "")
    db.commit()
    db.refresh(target)
    return _admin_user(target)
...
def set_verified(
    ...
    _audit(db, admin, target, "set_verified", str(payload.verified).lower())
    db.commit()
    db.refresh(target)
    return _admin_user(target)
