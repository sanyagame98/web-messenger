...
# Add the same animated collection picker to profile emoji status settings and
# render the currently selected status live, including local TGS statuses.
profile_path = ROOT / "src/components/sidebarLeft/tabs/editProfile.tsx"
profile = profile_path.read_text(encoding="utf-8")
profile_import = "import {mountRoofPremiumEmojiStatus, openRoofPremiumEmojiPicker} from '@lib/roof/RoofPremiumEmojiPacks';\n"
...
head_marker = """        <div class=\"roof-profile-premium-head\">\n          <div>\n            <div class=\"roof-profile-premium-title\">Roof Premium статус</div>"""
head_replacement = """        <div class=\"roof-profile-premium-head\">\n          <div class=\"roof-premium-status-preview\" ref={(el) => {\n            createEffect(() => void mountRoofPremiumEmojiStatus(el, roofStatus(), 38));\n          }} />\n          <div class=\"roof-premium-status-copy\">\n            <div class=\"roof-profile-premium-title\">Roof Premium статус</div>"""
if "roof-premium-status-preview" not in profile:
    if head_marker not in profile:
        fail("profile premium status head marker not found")
    profile = profile.replace(head_marker, head_replacement, 1)

# The previous replacement introduces one extra wrapper around the title/caption.
caption_marker = """            <div class=\"roof-profile-premium-caption\">\n              {roofPremium() ? `Premium активен · ${roofStars()} Roof Stars` : `${roofStars()} Roof Stars`}\n            </div>\n          </div>\n          <span class=\"roof-premium-badge\">★</span>"""
caption_replacement = """            <div class=\"roof-profile-premium-caption\">\n              {roofPremium() ? `Premium активен · ${roofStars()} Roof Stars` : `${roofStars()} Roof Stars`}\n            </div>\n          </div>\n          <span class=\"roof-premium-badge\">★</span>"""
if caption_marker not in profile:
    fail("profile premium status caption marker not found")
...
checks = {
    ...
    profile_path: "mountRoofPremiumEmojiStatus(el, roofStatus(), 38)",
...
print("[Roof premium emoji patch] ZIP packs, animated chat emoji and profile statuses installed")
